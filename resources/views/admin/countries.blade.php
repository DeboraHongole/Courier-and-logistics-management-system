    ...
    <section class="content-header">
        ...
        <div class="card card-info">
            <div class="card-header">
              <h3 class="card-title">Add New Countries</h3>
            </div>
            
            <form class="form-horizontal" method="POST" action="{{ route('admin.add_country') }}">
                       @csrf
              <div class="card-body">
                  <div class="row">
                      <div class="col-4">
                            <div class="form-group">
                                  ...
                                  <input type="text" name="Country" class="form-control @error('Country') is-invalid @enderror"  placeholder="Country Name" value="{{ old('Country') }}">
                ...
                <div class="row">
                  <div class="col-sm-12">
                    <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                      ...
                      @forelse($Coutry as $data)
                        <tr >
                            <td>{{$loop->index+1}}</td>
                            <td>{{$data->country_name}}</td>
                            <td>{{ date_format($data->created_at,'d-M-Y')}}.</td>
                            <td>{{count($data->getRegion)}}</td>
                           <!-- /.content-wrapper 

                           <td>
                              @foreach($data->getRegion as $a)
                              {{($a->getCountry->id)}}
                             @endforeach
                             </td>
